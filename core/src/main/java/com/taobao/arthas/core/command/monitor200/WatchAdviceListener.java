...
class WatchAdviceListener extends AdviceListenerAdapter {
	...
	private void watching(Advice advice) {
		try {
			...
			if (this.isVerbose()) {
				process.write(
						"Condition express: " + command.getConditionExpress() + " , result: " + conditionResult + "\n");
			}
			if (conditionResult) {
				...
				model.setClassName(advice.getMethod().getClazz().getName());
				model.setMethod(advice.getMethod().getMethodName());
			...
			process.end(-1, "watch failed, condition is: " + command.getConditionExpress() + ", express is: " + command
					.getExpress() + ", " + e.getMessage() + ", visit " + LogUtil.loggingFile() + " for more details.");
		}
	}
}
